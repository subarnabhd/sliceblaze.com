...
const Categorycount = () => {
  ...
  const fetchCategoryCounts = async () => {
    ...
    try {
      ...
      let categoryArray: CategoryCount[]
      ...
      <div className="bg-linear-to-b from-white to-gray-50 py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3 md:gap-4">
            {[1, 2, 3, 4, 5, 6].map((i) => (
              <div key={i} className="bg-white rounded-2xl shadow-sm p-5 border-2 border-gray-100 animate-pulse">
      ...
      <div className="bg-linear-to-b from-white to-gray-50 py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center py-12 bg-white rounded-2xl border-2 border-gray-200 shadow-sm">
            ...
            <p className="text-sm text-gray-400 mt-2">Check back soon for new businesses</p>
          </div>
        </div>
      </div>
    )
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          ...
          <p className="text-lg text-gray-600 max-w-2xl mx-auto">
            Find exactly what you&apos;re looking for. Browse businesses by category and discover local favorites.
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3 md:gap-4">
          {categoryCounts.map((item) => (
            ...
            >
              ...
              {item.image_url ? (
                <div className="relative h-32 w-full overflow-hidden">
                  ...
                  <div className="absolute bottom-3 right-3">
                    <div className="inline-flex items-center gap-1 bg-white/95 backdrop-blur-sm text-gray-900 px-2.5 py-1 rounded-full shadow-lg text-xs font-bold">
                      ...
                      <span className="opacity-70">{item.count === 1 ? 'place' : 'places'}</span>
                    </div>
                  </div>
                </div>
              ) : (
                ...
                <div className="relative p-5">
                  ...
                  <div className="relative inline-block mb-3 w-full text-center">
                    <div className="absolute inset-0 bg-red-50 rounded-full scale-0 group-hover:scale-110 transition-transform duration-300 mx-auto" style={{width: '64px', height: '64px', left: 'calc(50% - 32px)'}}></div>
              ...
              {/* Content Section */}
              <div className={`relative ${item.image_url ? 'p-4' : 'px-5 pb-5'}`}>
                {/* Category name */}
                <h3 className={`text-sm md:text-base font-bold group-hover:text-[#ED1D33] transition-colors mb-2 line-clamp-1 ${item.image_url ? 'text-gray-900' : 'text-gray-900 text-center'}`}>
                  {item.category}
                </h3>
                ...
                {!item.image_url && (
                  <div className="text-center">
                    <div className="inline-flex items-center gap-1.5 bg-linear-to-r from-[#ED1D33] to-red-600 text-white px-3 py-1.5 rounded-full shadow-md group-hover:shadow-lg group-hover:scale-105 transition-all">
                      <span className="text-base md:text-lg font-bold">{item.count}</span>
              ...
              <div className="absolute bottom-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity">
                <svg className="w-4 h-4 text-[#ED1D33]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
